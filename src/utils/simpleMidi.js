...
export const createMIDIFile = (melodyData, chordData, options = {}) => {
  ...
  if (includeMelody && melodyData && melodyData.notes && melodyData.notes.length > 0) {
    ...
    melodyTrack.push(
      0x00, // Delta time
      0xFF, 0x03, melodyNameBytes.length, ...melodyNameBytes // Track name meta event
    );

    // Program change (instrument)
    melodyTrack.push(
      0x00, // Delta time
      0xC0 | melodyChannel, melodyInstrument // Program change
    );
  ...
  if (includeChords && chordData && chordData.progression && chordData.progression.length > 0) {
    ...
    chordTrack.push(
      0x00, // Delta time
      0xFF, 0x03, chordNameBytes.length, ...chordNameBytes // Track name meta event
    );

    // Program change (instrument)
    chordTrack.push(
      0x00, // Delta time
      0xC0 | chordChannel, chordInstrument // Program change
    );
  ...
  if (includeBass && chordData && chordData.progression && chordData.progression.length > 0) {
    ...
    bassTrack.push(
      0x00, // Delta time
      0xFF, 0x03, bassNameBytes.length, ...bassNameBytes // Track name meta event
    );

    // Program change (instrument)
    bassTrack.push(
      0x00, // Delta time
      0xC0 | bassChannel, bassInstrument // Program change
    );
